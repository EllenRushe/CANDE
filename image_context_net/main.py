...
def main():
    ...
    cost_dict = {
            'mse': nn.MSELoss(), # Default is 'mean'.
            'sse': nn.MSELoss(reduction='sum')            
    }
    cost = cost_dict[params.cost]
    model_module = __import__('.'.join(['models', params.model_name]),  fromlist=['object'])
    print("EMBEDDING SIZE", train_data.get_embedding_size())
    model = model_module.net(embedding_size=train_data.get_embedding_size())
    ...
    for epoch in range(1, params.num_epochs + 1):
        train(model, device, train_loader, optimizer, epoch, cost)

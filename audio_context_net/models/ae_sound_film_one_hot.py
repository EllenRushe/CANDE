...
class net(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.en_2 = FiLMBlock(64, 64, context_size, context_size)
        self.en_3 = FiLMBlock(64, 8, context_size, context_size)
        self.de_1= FiLMBlock(8, 64, context_size, context_size)
        self.de_2 = FiLMBlock(64, 64, context_size, context_size)
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    print('Train Epoch: {}\tLoss: {:.6f}'.format(epoch, np.mean(np.mean(losses))))
...
def test(model, device, test_loader, checkpoint=None):
    ...
    with torch.no_grad():
        with tqdm(total=len(test_loader)) as progress_bar:
            for batch_idx, (data, context) in tqdm(enumerate(test_loader)):
                data = data.to(device)
                context = context.to(device)
                target = data.clone()
                output = model(data, context)
